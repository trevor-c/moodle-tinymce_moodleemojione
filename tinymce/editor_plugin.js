...
(function() {
    tinymce.create('tinymce.plugins.MoodleEmojione', {
        ...
        init : function(ed, url) {
            ...
            ed.addCommand('mceMoodleEmojione', function() {
                lang = ed.getParam('language');
                ed.windowManager.open({
                    file : ed.getParam("moodle_plugin_base") + 'moodleemojione/dialog.php?lang=' + lang ,
                    width : 640 + parseInt(ed.getLang('moodleemojione.delta_width', 0)),
                    height : 350 + parseInt(ed.getLang('moodleemojione.delta_height', 0)),
                    inline : 1
                }, {
            ...
            ed.onInit.add(function(ed) {
                ...
                this._emoticons = tinymce.util.JSON.parse(ed.getParam('moodleemojione_emoticons'));
            ...
            ed.onPreProcess.add(function(ed, o) {
                if (o.save) {
                    tinymce.each(ed.dom.select('img.emojione', o.node), function(image) {
                        var emoticontxt = '';
                        var matches = /^emojione/.exec(image.className);
						if (matches) {
                        	if (matches.length != 2) {
                        	...
                        	var index = matches[1];
                        	var search = new RegExp('class="emojione"');
                        	for (var emotxt in this._emoticons) {
            ...
            ed.addButton('moodleemojione', {
                title : 'moodleemojione.desc',
                cmd : 'mceMoodleEmojione',
                image : url + '/img/moodleemojione.png'
            });
        },
        ...
        getInfo : function() {
            return {
                longname : 'Moodle Emojione plugin',
                author : 'David Mudrak',
    ...
    // Register plugin.
    tinymce.PluginManager.add('moodleemojione', tinymce.plugins.MoodleEmojione);
})();
